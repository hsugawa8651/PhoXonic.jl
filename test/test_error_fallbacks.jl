...
@testset "Error Fallbacks" begin
    @testset "Shape dimension mismatch" begin
        ...
        @testset "2D lattice rejects 3D shapes" begin
            ...
            @test_throws ArgumentError Geometry(
                lat2d, air, [(Cylinder([0, 0, 0], 0.2, 0.5), air)]
            )
        ...
        @testset "3D lattice rejects 2D shapes" begin
            ...
            @test_throws ArgumentError Geometry(
                lat3d, air, [(Rectangle([0, 0], [0.3, 0.3]), air)]
            )
        end
    ...
    @testset "GFMethod fallback" begin
        ...
        @test_throws ArgumentError compute_greens_function(
            solver, k, ω_values, source, UnsupportedGF()
        )
        @test_throws ArgumentError compute_dos(solver, ω_values, k_points, UnsupportedGF())
        @test_throws ArgumentError compute_ldos(
            solver, position, ω_values, k_points, UnsupportedGF()
        )
    end
    ...
    @testset "Mid-level API fallbacks" begin
        @testset "solve_at_k" begin
            @test_throws ErrorException solve_at_k(
                "not a solver", [0.0, 0.0], DenseMethod()
            )
