...
geo_unit = Geometry(lat, Dielectric(1.0), [(Circle([0.5, 0.5], 0.2), Dielectric(9.0))])
...
# Plot
p = plot(; size=(700, 500))
plot!(p, ω_values, ldos_defect; label="Defect site (1.5, 1.5)", linewidth=2, color=:red)
plot!(
    p,
    ω_values,
    ldos_bulk;
...
# Mark the peak
vline!(
    p, [peak_ω]; linestyle=:dot, color=:gray, label="Peak at ω = $(round(peak_ω, digits=3))"
)
...
println(
    "Enhancement ratio:   $(round(maximum(ldos_defect) / maximum(ldos_bulk), digits=1))x"
)
