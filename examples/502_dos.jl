...
geo = Geometry(lat, Dielectric(1.0), [(Circle([0.5, 0.5], 0.3), Dielectric(9.0))])
...
for b in 1:size(freqs, 2)
    plot!(p, dists, freqs[:, b]; subplot=1, label="", linewidth=1.5, color=:blue)
end
...
for g in gaps
    hspan!(p, [g.max_lower, g.min_upper]; subplot=2, alpha=0.3, color=:lightgreen, label="")
end
