...
println(
    "  PSV: max |Dense - KrylovKit| = ", @sprintf("%.2e", max_diff_psv_krylov), " rad/s"
)
...
try
    ...
    max_diff_psv_lobpcg = maximum(
        abs.(bands_psv.frequencies .- bands_psv_lobpcg.frequencies)
    )
    rel_diff_psv_lobpcg = max_diff_psv_lobpcg / maximum(bands_psv.frequencies)
    println(
        "  PSV: max |Dense - LOBPCG| = ", @sprintf("%.2e", max_diff_psv_lobpcg), " rad/s"
    )
