...
solver = Solver(
    FullVectorEM(), geo, resolution, KrylovKitMethod(; shift=shift); cutoff=cutoff
)
...
solver_lobpcg = Solver(
    FullVectorEM(), geo, resolution, LOBPCGMethod(; shift=shift); cutoff=cutoff
)
...
if solver.basis.num_pw * 3 < 2000
    solver_dense = Solver(
        FullVectorEM(), geo, resolution, DenseMethod(; shift=shift); cutoff=cutoff
    )
