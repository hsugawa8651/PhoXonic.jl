...
function PhoXonic.compute_greens_function(
    solver::Solver,
    k,
    ...
    x_solutions, _ = ReducedShiftedKrylov.rscg(
        A,
        ComplexF64.(source),
        shifts;
        atol=method.atol,
...
function PhoXonic.compute_ldos(
    solver::Solver{Dim2},
...
function PhoXonic.compute_ldos(
    ...
    for k in k_points
        ...
        shifts = ComplexF64[ω^2 + im*η for ω in ω_values]
        G_values, _ = ReducedShiftedKrylov.rscg(
            A,
            ComplexF64.(source),
            shifts;
            atol=method.atol,
